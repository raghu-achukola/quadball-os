...
def main(location:str ,translate_roster:str = None):
    ...
    if translate_roster:
        translator = {
            (v['Team'],v['Jersey']):(v['Name'] if translate_roster == 'name' else v['ID'])
            for _, v in get_roster(rosters).T.to_dict().items()
        }
    sv = gen_statsheet_values(possessions, starter_cell= possessions.cell(7,1))
    for poss in sv:
        possession = StatSheetPossession.from_values(*poss)
        # print(possession.to_json())
...
if __name__ == '__main__':
    ...
    if tr:
        if tr.lower().strip() not in ('name', 'id'):
            ...
            tr = tr.lower().strip()
    main(args.statsheet_location, tr)